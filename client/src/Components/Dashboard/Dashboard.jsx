      ...
      <DashboardContainer>
        <LeftContainer>
          <div className="top_logo" onClick={() => navigate("/")}>
            ...
            <LogoText>KING DAVID ELITE</LogoText>
          </div>
              ...
              return (
                <>
                  {mainData.userData.accountType === 1 ? (
                    <>
                      {nav.title !== "Verification" && (
                        <div
                          key={i}
                          className={
                            index === i + 1
                              ? " whitespace-nowrap bg-[#0D0D0D] text-white rounded-md h-[full] flex gap-2 items-center px-4 py-2 text-[15px]"
                              : "item"
                          }
                          onClick={() => navigate(`/dashboard/${nav.link}`)}
                        >
                          <p>{nav.icon}</p>
                          <p>{nav.title}</p>
                        </div>
        ...
        <RightContainer>
          ...
          <div className="mainContent">{element}</div>
        </RightContainer>
      </DashboardContainer>
