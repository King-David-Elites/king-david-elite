...
const RealEstateListing = () => {
  ...
  return (
    <>
      {
        stage == 1 && <div className='p-2 fixed w-[100vw] top-0 bottom-0 h-[100%] bg-white transition-[1s] z-20'>
          <div className='p-4'>
            <div className="relative w-full lg:max-w-sm flex flex-col gap-6">
              <div className="h-[40px] flex items-center gap-4">
                <BiArrowBack size={30} className="opacity-[0.5]" onClick={() => setStage(0)} />
              ...
              <div className="flex gap-3 flex-wrap">
                {
                  views.map((v, i) => {
                    return (
                      <span className={`bg-gray-300 font-medium text-[15px] whitespace-nowrap h-full flex justify-center rounded-3xl p-3 ${view == v.title ? `rounded-md h-[10%]  bg-theme-color hover:opacity-[100%] items-center` : ""}`} key={i} onChange={(e) => setTitle(e.target.value)} onClick={() => { setQuery("view"); setView(v.title) }}>{v.title}</span>
                    )
                  })
                }
              </div>
